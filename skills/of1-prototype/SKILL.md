...
Generate pixel-perfect, self-contained HTML reproductions of key pages from the target website by invoking the `stardust:prototype` skill. These prototypes are the source-of-truth for the block-based EDS conversion (`of1-deploy`, which wraps `stardust:deploy`).
...
source material in the demo recreation pipeline. No redesign, no
...
EDS wraps content in `<div class="header-wrapper"><div class="header block">…</div></div>`. If a prototype uses `<header class="header">`, the EDS header block that `of1-deploy` generates inherits the collision and the nav renders wrong. Rename:
...
URLs MUST point at `/deliverables/prototype-*.html` (standalone HTML committed above), NOT the deployed EDS page URL produced later by `of1-deploy`.
