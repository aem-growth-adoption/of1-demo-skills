---
name: of1-extract-design-tokens
...
Delegate site-extraction to the `stardust:extract` skill, then publish the resulting brand-review HTML to the of1-demo-orchestrator repo for EDS hosting.
...
`OF1_EXTRACT_OWN_SITE=1` is set by the `of1-adopt-existing-site` orchestrator (never by `of1-demo-orchestrator`/`of1-demo-orchestrator-cc`, which always crawl an external domain).
...
Logo SVG completeness, deliverable image paths, and image format rules are documented in `of1-demo-orchestrator/knowledge/common-pitfalls.md` (§ 2-3).
