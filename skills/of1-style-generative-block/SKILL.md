...
- `DESIGN.json` — design tokens (colors, fonts, spacing, radius). **Resolve its path via `of1-demo-orchestrator/knowledge/design-tokens-resolution.md`** — it may live at `stardust/current/DESIGN.json` OR project root (`./DESIGN.json`, on a bounded-single `stardust:extract` run, as `of1-liftoff` runs it). Read whichever exists.
...
(Stage 2 `of1-liftoff` lifts pages onto the standard header/footer blocks for the e2e pipeline; the
existing site already has them for `of1-integration`), but do not assume a Stage 2 run always leaves
them in place — a failed or partial lift can skip the fragments. Do not assume; guarantee. This guard is
...
|---|---|
| `HEADER MISSING` / `FOOTER MISSING` | `/nav` or `/footer` doc is missing — re-run Step 4 (`ensure-nav-footer.mjs`); it authors a minimal branded fragment when `of1-liftoff`/the existing site didn't provide one |
| `OF1 BLOCK MISSING` | `blocks/of1/of1.js` wasn't pushed, or the `of1` block table's `th` cell doesn't read exactly `of1` |
| Screenshot shows unstyled links / system font | `styles/styles.css` (the site's own foundation CSS) didn't get pushed by `of1-liftoff`'s skin/deploy step, or the preview hasn't picked up the latest push yet |
