...
- Extraction outputs at `stardust/current/` (DESIGN.json, logo.svg, screenshots, per-page JSON — from step 3)
...
Step 3 wrote one JSON file per crawled page at `stardust/current/pages/<page>.json`, each listing the `images` array with the exact URLs as they appeared in the live DOM. That's the canonical source.
