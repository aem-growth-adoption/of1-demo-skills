...
- [~] `of1-build-templates` FIRST PASS done. Fixed: (1) intent-mode said base CSS "owned by assemble" — contradicts L41/36/78 (base owns it) → corrected; (2) "~5× slower" vs mode-table's "~3×" → unified to 3×. Assets glanced (deeper review = item 19): assemble-catalog.mjs solid (stylesheet-as-path matches schema, worker inlines HTML not CSS); fill-template.mjs already hardened (Finding 47/52/54), preview-only — cosmetic: itemKeys stops at item-6 but slots allow item-9 (only a log count). No dead vars (DOMAIN/OWNER/REPO/BRANCH all used). No playwright-cli use here (no §9 swap needed). Not full-audited: variation-slug guidance, sample-data ASCII rule enforcement.
