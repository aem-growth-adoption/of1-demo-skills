...
- [~] `of1-extract-content` FIRST PASS done. Fixed: stale "25 templates" at L199 → count-agnostic. FLAGS for user decision (not changed): (a) doc says images "up to 8" (L271) but download-images.mjs defaults --max-per-product=5 and the invocation doesn't override → real cap is 5 (still ≥4 gate). Fix doc to 5, or pass --max-per-product 8 for richer galleries (more uploads)? (b) testimonials.json is emitted here + claims worker consumption (L259) but is UNDOCUMENTED in worker-config-schemas.md and absent from its required/optional table — add to schema doc (if worker reads it) or soften the claim; needs worker-source confirmation. (c) minor: DA_TOKEN preflight (L24-25) only covers ADOBE_IMS_TOKEN/OF1_TOKEN_FILE, not the .hlx fallbacks in the contract's 4-step order → could false-fail when token lives only in .hlx (download-images would still find it). Verified: DA_TOKEN exported→used by child (not dead); playwright eval uses correct () => () form.
