...
- [~] `of1-generate-config-review` INITIAL REVIEW. Fixed: duplicate "### 2." step numbering (commit→3, verify→4). Script (fill-config-review.mjs) is solid — safeUrl/htmlEscape/formatPrice hardened, object-or-array normalization for every config, no functional bugs. KEY GAP (decision needed): the page's stated purpose is "review ALL config before deploy" but it renders neither faqs.json nor testimonials.json — both are deployed, worker-consumed configs (faqs vectorized for RAG; testimonials fill quote slots). Template has no tokens for them, script doesn't read them. Also the Step-1 gate REQUIRES faqs.json non-empty yet nothing renders it (inconsistent). Recommend adding Faqs + Testimonials sections (template tokens + render fns + stats) so the review surface matches what deploys. Covers asset item 17.
