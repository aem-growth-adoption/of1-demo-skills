 ...
 *
 * Adapted from of1-demo-orchestrator.js (Claude Code version) to match the of1-demo-orchestrator SKILL.md
 * architecture for SLICC's environment (sudoers, oauth-token, DA API, etc.)
 *
 * KEY DIFFERENCES FROM of1-demo-orchestrator.js (CC version):
 * ──────────────────────────────────────────────────
 ...
 *   - repo (optional): absolute path to clone. Defaults to /workspace/of1-demo-orchestrator.
 */

export const meta = {
  name: 'of1-demo-slicc',
  description: 'Full OF1 demo pipeline for SLICC — adapted from of1-demo-orchestrator skill',
...
const REPO_PATH = parsed.repo || '/workspace/of1-demo-orchestrator'
const OWNER = 'aem-growth-adoption'
const REPO_NAME = 'of1-demo-orchestrator'
...
- repo-config.json: /shared/of1-demo-orchestrator/repo-config.json
`

const OUTPUT_CONTRACT = (stepNum) => `
## Output Contract
Write a status file to /shared/of1-demo-orchestrator/step-${stepNum}-status.json:
...
${STAY_ON_TASK('Setup')}
${ENV_BLOCK(1, 'of1-check-dependencies')}
...
Write repo-config.json to /shared/of1-demo-orchestrator/repo-config.json with:
{
  "owner": "${OWNER}",
  "repo": "${REPO_NAME}",
  "branch": "${BRANCH}",
...
${ENV_BLOCK(2, 'of1-discover-narrative')}
...
${ENV_BLOCK(3, 'of1-extract-design-tokens')}
...
${ENV_BLOCK(4, 'of1-build-prototypes')}
...
${ENV_BLOCK(5, 'of1-convert-to-eds')}
...
Write status to /shared/of1-demo-orchestrator/step-8-content-status.json
`, { label: 'content-metadata', model: 'sonnet' }),

  // Track B: Step 10 — CTA template
  () => agent(`
${STAY_ON_TASK('CTA template')}
${ENV_BLOCK(10, 'of1-build-cta-template')}
...
${ENV_BLOCK(7, 'of1-style-generative-block')}
...
${ENV_BLOCK('6-base', 'of1-build-templates')}
...
Write status to /shared/of1-demo-orchestrator/step-6-base-status.json
...
${ENV_BLOCK(9, 'of1-build-quick-suggestions')}
...
${ENV_BLOCK(`6-${intent}`, 'of1-build-templates')}
...
Write status to /shared/of1-demo-orchestrator/step-6-intent-${intent}-status.json
...
1. cd ${REPO_PATH}
2. Run: python3 /workspace/skills/of1-build-templates/assets/assemble-catalog.py . ${OWNER} ${REPO_NAME} ${BRANCH}
3. mkdir -p gallery drafts tools
4. cp /workspace/skills/of1-build-templates/assets/gallery.html gallery/index.html
...
Write: /shared/of1-demo-orchestrator/step-6-status.json with deliverable URL ${PREVIEW_BASE}/gallery/index.html
...
3. Run: python3 /workspace/skills/of1-generate-config-review/assets/fill-config-review.py . ${DOMAIN}
...
Write: /shared/of1-demo-orchestrator/step-11-status.json with deliverable URL ${PREVIEW_BASE}/deliverables/config-review.html
